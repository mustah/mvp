...
public class MeasurementRepository extends QueryDslJpaRepository<MeasurementEntity, Long> {
  ...
  private final PathBuilder<MeasurementEntity> builder;
  private final Querydsl querydsl;

  @Autowired
  MeasurementRepository(EntityManager entityManager) {
    this(entityManager, DEFAULT_ENTITY_PATH_RESOLVER);
  }

  private MeasurementRepository(EntityManager entityManager, EntityPathResolver resolver) {
    this(new JpaMetamodelEntityInformation<>(MeasurementEntity.class, entityManager.getMetamodel()),
  ...
  public Page<MeasurementEntity> findAllScaled(String scale,
    ...
    query.select(
        Projections.constructor(
            ...
            queryMeasurement.id, queryMeasurement.created, queryMeasurement.quantity,
            Expressions.simpleTemplate(
                MeasurementUnit.class, "unit_at({0}, {1})", queryMeasurement.value, scale),
            queryMeasurement.physicalMeter
        )).where(predicate).from(path);
