          ...
          from physical_meter pm
                 join (select physical_meter.id,
                              generate_series(case
                                                when date_trunc('day', lm.created) <
                                                     date_trunc('day', (current_timestamp - interval'3 months'))
                                                then (current_date - interval'3 months') AT TIME ZONE 'UTC' AT TIME ZONE 'UTC+1' AT TIME ZONE 'UTC'
                                                else date_trunc('day', lm.created::timestamp) AT TIME ZONE 'UTC' AT TIME ZONE 'UTC+1' AT TIME ZONE 'UTC'
                                              end,
                                              current_date,
           ...
           left join (select distinct physical_meter_id, created
                      from measurement
                      where created > current_timestamp - interval'3 months'
