...
public class CompatibilityFunctionsTest {
  ...
  public void jsonbContainsArray() {
    ...
    assertTrue(jsonbContains("[1,2,3]", "[3,1,2]")); //order is not significant
    assertTrue(jsonbContains("[1,2,3]", "[3,1,1,1,2]")); //duplicate elements don't matter
    assertFalse(jsonbContains("[1,2,3]", "[1,3,77]"));
    //special exception!
    assertTrue(jsonbContains("[\"foo\", \"bar\"]", "\"bar\""));
    assertFalse(jsonbContains("\"bar\"", "[\"foo\", \"bar\"]"));
  }
