...
public interface MeasurementJpaRepository
  ...
  @Query(nativeQuery = true, value = 
    ...
    + "              AND measurement_inner.physical_meter_id = measurement_outer.physical_meter_id "
    + "              AND inner_date_serie.date < date_serie2.date "
    + "            ORDER BY inner_date_serie.date DESC "
    + "            LIMIT 1 "
    + "          ) "
    + "        ) AS value, "
    + "    date_trunc(:resolution, created AT TIME ZONE 'UTC') AT TIME ZONE 'UTC' AS interval_start"
    + "  FROM ( "
    + "    SELECT  "
    + "      generate_series( "
    + "        date_trunc(:resolution, "
